...
## After a reboot: two scripts, nothing else
...
Robot first, laptop second. Ctrl+C either one to stop everything it started; shut the
laptop down before the robot so the base watchdog halts the base itself.

`start_robot.bash` lives in this repo but **runs on the robot** — copy it to
`~/tams_ws/` (or anywhere on the robot) after changing it:

```bash
scp start_robot.bash tmr-user@companion:~/start_robot.bash
```

Neither script moves anything. The arm impedance controllers spawn inactive, and sending
the arms to the home pose stays manual — see *Start both arms from the home pose*, or
`RUNBOOK.md` §5 for the condensed version.

`start_robot.bash` also refuses to start on top of a running stack. That is not politeness:
a leftover `controller_manager` answers the new launch's spawners, which then report
`Controller already loaded` and fail to configure. See *Robot: a controller spawner dies
with "Failed to configure controller"* below.

### Start all laptop-side teleoperation

The all-in-one launcher always starts and verifies the pedal launch before starting GELLO:
...
It checks for two foot switches and two GELLO serial devices, configures DDS, and stops both
launch processes together on Ctrl+C. On the TAMS laptop it automatically enters the running
`teleop_inputs` ROS 2 container; no host ROS or Pixi setup is required.
It also pings the robot and checks the robot-vs-laptop clock skew from
`/swerve_drive_controller/odom` stamps, refusing to start beyond 0.4 s — the base
watchdog would otherwise discard every command silently. (The ssh-based skew check
further up needs a password; this one does not.) `--skip-device-checks` skips **all**
preflight and is available for diagnostics.
Arm-controller activation intentionally remains manual because both arms must first be
verified at the recorded home pose.
...
`ros2 topic hz` and `ros2 node list` are hit by this too, and unsetting the profile does
**not** reliably bring the local nodes back — observed on 2026-08-20 with a fully healthy
stack: `ros2 node list` showed only the robot's nodes, and `ros2 topic hz /pedal/state`
reported NO DATA, while the pedals and both GELLOs were publishing normally.

**Do not judge a laptop-local stack by `node list` or `topic hz`.** Use `ros2 topic info`,
which reads the endpoint registry rather than waiting for a sample, plus the process list:

```bash
ros2 topic info /pedal/state                 # want: 1 publisher, 2 subscribers
ros2 topic info /left/gello/joint_states     # want: 1 publisher
pgrep -af 'install/(pedal_state_publisher|tmr_pedal_teleop|franka_gello_state_publisher)'
```

To prove the robot link itself is alive, read a robot-published topic — that path is not
affected. `start_teleop.bash`'s preflight does exactly this with
`/swerve_drive_controller/odom`.

## Robot: a controller spawner dies with "Failed to configure controller"

Signature (observed 2026-08-20 on `joint_state_broadcaster`):

```
[spawner-4] WARN : Controller already loaded, skipping load_controller
[spawner-4] ERROR: Failed to configure controller
[ERROR] [spawner-4]: process has died ... exit code 1
```

**The cause is a leftover bringup, not the controller's configuration.** The tell is that
the freshly started `controller_manager` never logs `Loading controller '<name>'` for it —
the spawner reached a *previous* bringup's manager, where the controller was already
loaded and **active**, and configuring an active controller is invalid.

Do not go looking for missing `joints:`/`interfaces:` parameters; the controller never got
far enough to claim an interface.

Fix: make sure the old stack is really gone, then either relaunch or spawn in place —
the manager does not need restarting, and a broadcaster claims only state interfaces, so
it cannot move anything.

```bash
pgrep -af 'ros2_control_node|tmrv0_2.launch'
ros2 node list | grep -E 'controller_manager|joint_state_broadcaster'
ros2 run controller_manager spawner joint_state_broadcaster -c /controller_manager
```

`start_robot.bash` guards against this by refusing to start on top of a running stack.

Consequences while it is missing are cosmetic: `/dynamic_joint_states` has no publisher,
and `/joint_states` carries `joint_state_publisher`'s **fake URDF values** rather than real
wheel readings. Teleoperation is unaffected — `swerve_drive_controller` reads its state
interfaces directly, not through either topic.

## Robot: arm spawner dies FATAL "Failed loading controller" after a 10 s timeout

Different cause from the stale-manager case above, same family. Signature:

```
[right.spawner_joint_state_broadcaster]: Failed getting a result from calling
    /right/controller_manager/load_controller in 10.0. (Attempt 1 of 3.)
[right.controller_manager]: A controller named 'joint_state_broadcaster' was already loaded
[right.spawner_joint_state_broadcaster]: FATAL: Failed loading controller joint_state_broadcaster
```

Read it in order: the spawner's first `load_controller` call **succeeded on the server**
but its response did not arrive within 10 s, so the spawner retried, and the retry
correctly reported the controller as already loaded. The manager was simply too busy —
observed on 2026-08-20 when the gripper launch (12 processes, two more
`controller_manager`s, six spawners) started while the arm managers were still spawning.

**The damage is a half-finished controller.** The spawner died before configuring it, so
it sits `unconfigured` rather than `inactive`, and activating it later fails. The two arms
end up asymmetric — exactly what happened:

```
left  joint_impedance_controller  unconfigured   <- activation would fail
right joint_impedance_controller  inactive       <- correct
```

Check and repair (a state transition only — claims no command interfaces, moves nothing):
...
`inactive` is the **correct** resting state here — `joint_impedance_controller` spawns
`--inactive` by design so activation stays an explicit gate.

`start_robot.bash` avoids the race by waiting for `joint_impedance_controller` to actually
reach `active`/`inactive` on both arms before launching the grippers, and repairs any
controller it finds left `unconfigured`. Waiting for the controller_manager's *services*
is not enough — those appear almost immediately, long before the spawners finish.

## Robot: arm aborts with `communication_constraints_violation`

```
[ros2_control_node-2] [ERROR] [FrankaHardwareInterface]: libfranka: Move command aborted:
    motion aborted by reflex! ["communication_constraints_violation"]
```

**This is a robot-side reflex, not a controller fault.** The FCI expects a command packet
every 1 ms; when enough arrive late in a rolling window the arm aborts the motion itself.
The tell that it is timing and not a crash is that everything else keeps running — the
gripper managers go on accepting goals half a second later.

Get moving again (one arm; the controller must be re-activated afterwards):
...
Then find out which of the four causes fired — the ROS log never says:

```bash
./configs/check_realtime.sh          # from the laptop; --local on the companion
```

### 1. Memory locking denied (`VmLck: 0 kB`)

`ros2_control_node` calls `lock_memory()` and logs `Unable to lock the memory: 'No proper
privileges to lock the memory!'` when the `memlock` limit is too low. **This is not benign
for a 1 kHz loop**, despite being harmless for the base. FIFO priority stops other
*processes* preempting the control thread; it does nothing about a page fault *inside* it,
which stalls the loop for milliseconds regardless of priority. This is the only cause that
fires on an otherwise idle, correctly prioritised machine.

```bash
./configs/check_realtime.sh --fix    # writes /etc/security/limits.d/99-tmr-realtime.conf
```

Limits are granted at **login**. Close every terminal running a robot stack and open new
ones — re-sourcing or `exec bash` inherits the old limits — then re-run the check.

### 2. No FIFO scheduling at all

If the `rtprio` limit is 0, `configure_sched_fifo()` fails and the loop runs `SCHED_OTHER`,
competing with a box that sits at load 12–14 on 12 cores with the full stack up. The same
`--fix` covers this. `controller_manager` logs `Could not enable FIFO RT scheduling policy`
when it happens.

### 3. Not enough CPU headroom

Two arms at 1 kHz, the base, the spine and two gripper `controller_manager`s share the
companion. `tmr_duo_config.yaml` sets `use_visualization: "false"` to claw back ~1.6 cores:
`robot_state_publisher` and `joint_state_publisher` only produce TF for a viewer, and
nothing in the teleop path reads them (`joint_impedance_controller` uses ros2_control state
interfaces; `gello_calibration_check.py` reads `/<side>/franka/joint_states` from
`joint_state_broadcaster`). The aggregated `/<side>/joint_states` has no consumer at all.

Set it back to `"true"` if you want TF for rviz — expect the loops to get tighter.

### 4. FCI shares a link with all DDS traffic

Both arms (`172.16.16.12` / `.11`), the spine (`.10`) and the teleop laptop are on one flat
`/24`, so FastDDS discovery multicast and joint-state bursts queue alongside the 1 kHz
control stream. Franka wants FCI on a dedicated interface for exactly this reason.

Non-zero `dropped`/`overrun` in the check's interface section confirms it. Short of
rewiring, disable interrupt coalescing on that NIC — **with the robot idle**, it bounces
the link:

```bash
sudo ethtool -C <dev> rx-usecs 0 rx-frames 1
```

### And if all four are clean

Check the kernel. Franka requires **PREEMPT_RT** for 1 kHz control; on a generic kernel the
tail of the scheduling-latency distribution reaches into the milliseconds and the reflex
fires occasionally no matter how the limits are set.

> Not this failure: `joint_impedance_controller` used to call `rclcpp::shutdown()` from the
> 1 kHz thread on a GELLO timeout, which blocked long enough to trip the same reflex. That
> is fixed — it now returns `ERROR` instead. That path logs a GELLO timeout first, so if no
> such line precedes the abort, this is not it.

## Robot: `franka_robot_state_broadcaster` will not activate

```
Can't activate controller 'franka_robot_state_broadcaster':
    State interface with key 'fr3/robot_state' does not exist
```

An `arm_id` mismatch. The broadcaster asks for `fr3/robot_state`, but this robot exports
`left_fr3v2/robot_state` and `right_fr3v2/robot_state` — `tmr_duo_config.yaml` sets
`arm_id: fr3v2` (see *Arm teleoperation*), and that value is not reaching the broadcaster.
On the left side it also fails earlier, with `Missing namespace : /left or wildcard in
parameter file`.

Confirm what the hardware really exports by reading `joint_state_broadcaster`'s claimed
interfaces — it takes all of them, so it lists the true names:

```bash
ros2 control list_controllers -v -c /left/controller_manager
```

**Harmless for teleoperation.** The controller stays inactive and its
`/<side>/franka_robot_state_broadcaster/*` topics never publish, but nothing in the GELLO
path reads them: `joint_impedance_controller` uses ros2_control state interfaces directly.
The fix belongs in the robot's `franka_fr3_arm_controllers` launch/config in `~/tams_ws`,
not in this repository.

## Robot: rviz2 dies immediately with exit code -6

```
qt.qpa.xcb: could not connect to display
```

No X display, e.g. the base launch started over plain `ssh`. The SIGABRT follows from Qt
failing to initialise, and it is harmless — rviz is only a viewer. Use `ssh -X`, or let
`start_robot.bash` handle it: with no `DISPLAY` set it exports `QT_QPA_PLATFORM=offscreen`
so rviz starts headless instead of aborting.

Also in the same log: `Unable to lock the memory: 'No proper privileges to lock the
memory!'`. RT memory locking was denied, but the manager still gets FIFO priority 98.

> Benign **for the base only.** The same denial on an arm's `controller_manager` is a real
> fault: an unlocked 1 kHz FCI loop can be stalled past its deadline by a page fault no
> matter what priority it holds. See *arm aborts with `communication_constraints_violation`*.
