...
# use_visualization: Start robot_state_publisher/joint_state_publisher (default: 'true')
#
...
def generate_robot_nodes(context):
    ...
    # The spawner runs inside /left or /right. A bare controller key is rejected
    # for a namespaced spawner, so use ROS wildcard parameter form. Each arm gets
    # its own temporary file, keeping the arm-specific hardware identifier isolated.
    broadcaster_params = {
    ...
    # robot_state_publisher and joint_state_publisher exist only to produce TF for a
    # viewer. Nothing in the teleoperation path reads either of their outputs:
    # joint_impedance_controller takes ros2_control state interfaces directly, and the
    # tools that do read a topic (gello_calibration_check.py) read
    # /<side>/franka/joint_states, which joint_state_broadcaster publishes. The aggregated
    # /<side>/joint_states has no consumer at all.
    #
    # They are not free. joint_state_publisher is Python and burns ~80% of a core per
    # instance; with two arms that is 1.6 cores plus its DDS traffic, on a companion that
    # already sits at load 12-14 on 12 cores with the full stack up. That headroom is what
    # the 1 kHz FCI loops need to make their deadlines, and losing it shows up as
    # "communication_constraints_violation" reflexes. Default stays "true" so this launch
    # file behaves like upstream; tmr_duo_config.yaml turns it off for headless teleop.
    visualization = IfCondition(LaunchConfiguration("use_visualization"))
...
def generate_launch_description():
    launch_args = [
        ...
        DeclareLaunchArgument(
            ...
            default_value="true",
            description=(
                "Start robot_state_publisher and joint_state_publisher. They produce TF "
                "for a viewer and nothing else; teleoperation does not read them. Set "
                "false on a loaded companion to give the 1 kHz FCI loops their headroom."
            ),
        ),
    ]
