...
controller_interface::return_type JointImpedanceController::update(
  ...
  if (move_to_start_position_finished_) {
    ...
    if (!hasFreshGelloState_()) {
      // Never shut down rclcpp from the 1 kHz update thread. Shutdown can block long enough
      // for libfranka to miss its communication deadline and trigger a
      // communication_constraints_violation. Returning ERROR lets controller_manager stop
      // this controller while its hardware loop remains alive and schedulable.
      for (int i = 0; i < num_joints; ++i) {
        command_interfaces_[i].set_value(0.0);
      }
...
CallbackReturn JointImpedanceController::on_init() {
  try {
    ...
    // on_configure reads k_alpha. Without this declaration it only resolves because
    // ros2_control builds controller nodes with automatically_declare_parameters_from_overrides
    // and controllers.yaml happens to supply it; drop that key and .as_double() throws out
    // of a lifecycle callback, which deadlocks controller_manager the same way the
    // robot_description fetch used to.
...
CallbackReturn JointImpedanceController::on_configure(
  ...
  // NO robot_description fetch here, deliberately. This used to build an
  // AsyncParametersClient against "robot_state_publisher" and call wait_for_service()
  // with no timeout, then future.get(). The value went into robot_description_, which
  // nothing ever read.
  //
  // That cost the whole robot. "robot_state_publisher" is a RELATIVE name, so it
  // resolves to <ns>/robot_state_publisher - and tmr_duo_config.yaml sets
  // use_visualization "false", so franka.launch.py does not start one. controller_manager
  // holds services_lock_ across on_configure, so the un-timed wait deadlocked
  // list_controllers / configure_controller / switch_controller / load_controller for the
  // life of the process. Parameter services sit on the node's default callback group and
  // kept answering, which is why the node looked alive while every `ros2 control` command
  // timed out. Arm teleoperation could not be activated at all.
  //
  // If the URDF is ever genuinely needed, take it as a controller parameter - never via a
  // blocking round-trip to another node from inside a lifecycle callback.
  // franka.launch.py already passes robot_description to the controller_manager node.
...
CallbackReturn JointImpedanceController::on_activate(
  ...
  if (!hasFreshGelloState_()) {
    RCLCPP_ERROR(get_node()->get_logger(),
                 "Refusing activation: no valid GELLO joint state received in the last 0.5 s");
    return CallbackReturn::ERROR;
  }
...
bool JointImpedanceController::validateGelloJointState_(
  ...
  if (timestamp_age >= max_gello_state_age_seconds) {
    RCLCPP_WARN_THROTTLE(logger, *clock, 2000,
                         "Rejecting GELLO state: message too old "
                         "(timestamp age %.6f s, limit %.3f s)",
                         timestamp_age, max_gello_state_age_seconds);
    return false;
  }
  return true;
}
...
bool JointImpedanceController::hasFreshGelloState_() const {
  ...
  return reception_age >= 0.0 && reception_age < max_gello_state_age_seconds;
}
