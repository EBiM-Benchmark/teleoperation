...
=======
    def _on_mode(self, msg):
        ...
        if self.mode == DRIVE:
            self.get_logger().info(
                f"base_bridge mode -> {self.mode}: pedals move the base again."
            )
        else:
            # Warn, not info: from here the base stops responding while every diagnostic
            # still looks healthy, which is exactly the state that wastes debugging time.
            self.get_logger().warn(
                f"base_bridge mode -> {self.mode}: BASE HELD STILL. cmd_vel keeps "
                "publishing at 20 Hz but all-zero, so the pedals will not move the base "
                "until you press 'm' to return to DRIVE."
            )
