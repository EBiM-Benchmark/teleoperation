...
class PedalStatePublisher(Node):
    ...
    def __init__(self):
        ...
        missing = [index for index, device in self.devices.items() if device is None]
        if missing:
            for device in self.devices.values():
                if device:
                    try:
                        ...
                        pass
            raise RuntimeError(
                f"Both foot switches are required; missing or busy switch(es): {missing}. "
                "Check for another pedal publisher and verify device access."
            )
