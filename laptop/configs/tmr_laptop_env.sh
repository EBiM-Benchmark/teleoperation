...
# because it is a direct link, but it is not always present: the GELLO host (ebimHP) has
# never had one, and this laptop lost its wired link on 2026-08-23. Both machines still
# share the 192.168.50.x LAN, and `ros2 multicast send/receive` was verified across it,
# so fall back there rather than refusing to start.
#
